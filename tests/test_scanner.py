...
class ScannerTests(unittest.TestCase):
 ...
 def test_bearish_candidate_has_stop_above_entry_and_descending_targets(self):
  row={"instrument_id":"NIFTY","price":100,"tradable":True,"liquidity":80,"quality":80,"coa":80,"trend_score":20,"momentum":70,"volume":70,"relative_strength":70,"risk":80,"volatility":70}
  candidate=OpportunityScanner().rank([row])[0]
  self.assertEqual(candidate.trend,"BEARISH")
  self.assertGreater(candidate.stop_zone,candidate.entry_zone)
  self.assertGreater(candidate.entry_zone,candidate.target1)
  self.assertGreater(candidate.target1,candidate.target2)
