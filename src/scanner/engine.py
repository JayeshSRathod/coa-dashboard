...
class OpportunityScanner:
 ...
 def rank(self, universe:Sequence[Mapping[str,object]], horizon:str="SWING")->list[DecisionCandidate]:
  ...
  for row in universe:
   ...
   price=float(row.get("price",0)); trend="BULLISH" if float(row.get("trend_score",0))>=50 else "BEARISH"
   if trend=="BULLISH":
    stop,target1,target2=round(price*.97,2),round(price*1.03,2),round(price*1.06,2)
   else:
    stop,target1,target2=round(price*1.03,2),round(price*.97,2),round(price*.94,2)
   out.append(DecisionCandidate(str(row["instrument_id"])+":"+horizon,horizon=horizon,instrument_id=str(row["instrument_id"]),score=score,trend=trend,confidence=score,entry_zone=price,stop_zone=stop,target1=target1,target2=target2,coa_alignment=float(row.get("coa",0)),created_at=datetime.now(timezone.utc).isoformat()))
